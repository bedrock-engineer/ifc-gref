...
import type { IfcMetadata } from './types'

export type LogeorefLevel = 'le10' | 'l20' | 'l50'

export function detectLogeoref(metadata: IfcMetadata): LogeorefLevel {
  if (metadata.existingGeoref) return 'l50'
  if (metadata.siteReference) return 'l20'
  return 'le10'
}

/** Human-readable label for the level, e.g. "LoGeoRef 50". */
export function logeorefLabel(level: LogeorefLevel): string {
  switch (level) {
    case 'le10':
      return 'LoGeoRef ≤ 10'
    case 'l20':
...
export function logeorefDescription(level: LogeorefLevel): string {
  switch (level) {
    case 'le10':
